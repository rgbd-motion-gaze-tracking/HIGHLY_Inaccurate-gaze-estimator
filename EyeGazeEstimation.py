...
# Gaze Estimation
def estimate_gaze(x,y,eye,screen,dist_screen):
    #print('pupil x-cord',x)
    #print(y)
    #print('eye_box_width',eye[2])
    h_scale = (screen[0]/eye[2])*(dist_screen)
    #print('h scale',h_scale)
    #v_scale = (screen[1]/eye[3])*(1/dist_screen)
    #print('eye width - pupil_x',eye[2]/2-x)
    x_monitor = (((eye[2]/2)-x)*h_scale)+(screen[0]/2)
    #y_monitor = (y-eye[3])*v_scale+(screen[1]/2)
    print('monitor_x_pos',x_monitor)
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        face_image = img[y:y + h, x:x + w] # Seperating face from the background
        ...
        for eye in eyes:
            ...
            if eye[0]<100:
                ...
                eye_image = img[faces[0][1]+eye[1]+12 : faces[0][1]+eye[1]+eye[3]-12, faces[0][0]+eye[0]+6 : faces[0][0]+eye[0]+eye[2]] # Shortening the eye box to avoid eye lashes, eye brows, etc.
                if eye_image.size ==0: # The code runs into error if empty image is passed into blob detector, to avoid that we use this if-else statement
                    ...
                    for keyPoint in keypoints: # Obtains keypoints in readable format
                        ...
                        estimate_gaze(x,y,eye,screen,dist_screen)
    ...
    key=cv2.waitKey(100) # Parameter is in miliseconds
